...
const APP_VERSIONS = [
  ...
  {
    version: "v1.7.0", date: "16 May 2026", type: "PREV",
    title: "Unified Rating System & Moderator Performance Dashboard",
    summary: "StarRating.tsx shared component. Captain rates passenger (blind submission). ModPerformance dashboard at /admin/mod-performance. ModQueue reviewed_at timestamps."
  },
  {
    version: "v1.8.0", date: "17 May 2026", type: "FEATURE",
    title: "ENB Trades Ecosystem — Phase 1",
    summary: "(1) Directory restructured: DirectoryHub at /directory with Business Directory and Trades Directory as two visual tiles. (2) TradesDirectory.tsx: emoji-led card grid with trade type filter chips, availability filter, rating filter, list and map view. Karachi default map centre. (3) TradesProfile.tsx: full digital resume — before/after photo portfolio of verified jobs, customer ratings with would-hire-again, availability status, weekly schedule display, Start a Job button, ENB membership benefits for non-members. (4) AvailabilityPicker.tsx: shared component, two tabs — Right Now (Available Now with 1h/2h/3h/Today duration, Busy) and Weekly Schedule (day-by-day From/To JSONB). (5) StartJobModal.tsx: generates 6-char Job Code for street encounters, 30-min expiry, QR code + copy link. (6) JobConfirmation.tsx: public page at /job/:code — customer confirms job, enters name + required mobile + problem description + optional price. ENB membership benefits pitch for non-members. (7) JobRating.tsx: public page at /job/:code/rate — emoji face + 5-star rating + would hire again Yes/No + optional comment. All fields stored in DB. (8) Profile.tsx: trades section added — availability setter, trade type chips, verified jobs count, link to own trades profile. (9) BusinessDirectory.tsx: map centre fixed Rawalpindi → Karachi. DB: job_requests table + 8 new columns on users (avg_job_rating, total_job_ratings, total_verified_jobs, trade_types, trade_availability, trade_availability_until, trade_availability_updated_at, trade_availability_schedule). update_tradesperson_stats SECURITY DEFINER RPC."
  },


  {
    version: "v1.9.0", date: "18 May 2026", type: "PREV",
    title: "GPS Verification System — All 5 Lapses Fixed",
    summary: "(1) Lapse 1: GPS accuracy captured from pos.coords.accuracy, stored in submissions.gps_accuracy_m, submissions with >100m accuracy (WiFi/indoor signal) forced to human moderation regardless of AI verdict — accuracy badge shown to user during submission. (2) Lapse 2: GPS coordinates, accuracy, submission time, and user trust level (verified submission count) passed to Gemini AI on every review — Gemini now performs indoor/outdoor inconsistency detection, time-of-day fraud analysis, and trust-weighted decisions. (3) Lapse 3: 787 Pakistan boundary polygons loaded from OpenStreetMap — 166 districts + 596 towns/tehsils + 25 neighbourhoods covering all of Pakistan, not just Karachi. PostGIS ST_Contains point-in-polygon check on every submission via check_point_in_boundary RPC. Submissions outside registered area flagged for human review. (4) Lapse 4: Embedded Leaflet map tile added to ModQueue — moderators see a 160px OpenStreetMap pin at exact submission coordinates with accuracy circle (amber = poor, green = good), plus GPS flag badges for poor accuracy, duplicate location, and boundary violations. (5) Lapse 5: Duplicate GPS detection — Haversine distance check against user's last 30 days of same action_type submissions, within 10m radius → gps_duplicate_flag = true → human review. (6) GPS constants taxonomy locked in beforeAfter.ts as single source of truth: MAX_GPS_DRIFT_METRES=20 (Before/After drift), GPS_ACCURACY_THRESHOLD_M=100 (signal quality), GPS_DUPLICATE_RADIUS_M=10 (repeat fraud). (7) Gemini prompts rebuilt v1.8.0: 9 action-specific prompts with explicit fraud pattern sections, Pakistani urban context, per-action confidence calibration, user trust tiers, time-of-day suspicion rules. AI-first moderation targets 90%+ autonomous decisions. DB: gps_accuracy_m (numeric) + gps_duplicate_flag (boolean) added to submissions. neighbourhood_boundaries table + PostGIS spatial index created."
  },
  {
    version: "v2.0.0", date: "20 May 2026", type: "CURRENT",
    title: "Test/Genesis Schema Separation + Environment Architecture",
    summary: "(1) GENESIS BLOCK: Public schema wiped clean — Day 1 of authentic civic action recording. Zero test submissions in the real environment. (2) TEST SCHEMA: All 46 development submissions + 23 test users preserved permanently in dedicated PostgreSQL test schema — full parallel environment with identical RPCs, RLS policies, triggers, and 787 GPS boundary polygons. Test data is the permanent QA record and proof that every feature works. (3) DUAL CLIENT: supabase.ts exports supabase (public) + supabaseTest (test schema) clients. getDb() returns correct client based on user.environment. (4) ENVIRONMENT ROUTING: App.tsx auth handler checks public.users first — if row found, environment=real. If no row found, checks test.users and routes test-only users (Category B) to test schema automatically. Shared auth.users reality handled transparently. (5) AMBER BANNER: TestEnvironmentBanner fixed at top (z-index 9999, #F59E0B, 36px) appears on EVERY screen when isTestEnvironment=true. Layout and DesktopSidebar offset 36px to clear banner. (6) ENVIRONMENT TOGGLE: Super-admin-only toggle in DesktopSidebar — switches between Real (green) and Test (amber) environments. Resets to Real on every new login. (7) CLOUDINARY PATHS: Before-photos route to enb/real/submissions/before or enb/test/. After-photos to enb/real/submissions/after or enb/test/. (8) SCHEMA DRIFT PREVENTION: Every future SQL migration applied to both schemas simultaneously. Monthly Super Audit includes schema sync check. GENESIS MEMBERS: 10 real users seeded.",
  },
];
