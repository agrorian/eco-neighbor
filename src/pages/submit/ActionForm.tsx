  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <h2 className="text-xl font-bold text-enb-text-primary capitalize">{actionType.replace(/_/g, ' l('submit', 'takePhoto')bg-enb-green/10 border-enb-green/20 text-enb-green' : 'bg-white border-gray-200 text-gray-500 cursor-pointer hover:bg-gray-50'}`}
        >
          {loadingLocation ? <Loader2 className="w-5 h-5 animate-spin" /> : <MapPin className="w-5 h-5" />}
          <span className="text-sm font-medium">
            {gpsAddress ? `📍 ${gpsAddress}` : loadingLocation ? 'Detecting...' : 'Tap to detect GPS locationl('submit', 'description')submit', 'descriptionPlaceholderl('submit', 'charsRemaining')bg-enb-green text-white border-enb-green' : 'bg-white border-gray-200 text-enb-text-primary hover:border-enb-green'}`}
            >
              {opt}
            </button>
          ))}
